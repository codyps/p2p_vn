 ...
 * (at runtime)
 *
 * Note: BPF_LD converts things to host byte order.
 */
struct sock_filter sf[] {
	...
	BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, ETH_P_IP, 1, 5),

	/* load ip src */
	BPF_STMT(BPF_LD | BPF_W | BPF_ABS, ETH_HLEN + offsetof(struct iphdr, saddr)),

	/* And it to compare for subnet */
	BPF_STMT(BPF_ALU | BPF_AND, __SUBNET_MASK__),

	/* is ipsrc == the filtered address? */
	BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, __SUBNET_VAL__, 1, 2),
...
int main(int argc, char **argv)
{
	...
	ret = ioctl(sock, SIOCGIFNETMASK, &req);
